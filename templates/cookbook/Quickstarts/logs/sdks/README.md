...
* **日志采集页面**：进入「日志采集 → 采集详情 → 查看数据」，跳转至日志检索页面（自动切换至对应索引集），输入关键词即可查看到上报的日志。

* **日志检索页面**：进入「日志平台 → 检索」，在检索条件中手动切换至对应的索引集，输入关键词即可查看到上报的日志。
...
2. **检查时间戳**
   * 确认 SDK 代码中 `timeUnixNano` 设置为当前时间附近的纳秒时间戳，时间偏差过大会导致数据检索不到。
   * 获取纳秒时间戳的方法示例：`Python: int(time.time_ns())`、`Java: System.currentTimeMillis() * 1_000_000`、`Go: time.Now().UnixNano()`。

3. **检查索引集匹配**
   * `service.name` 不能随意设置。该值必须与新建数据源时「采集项详情 → 字段提取 / 映射规则」中配置的规则一致，否则日志无法路由到目标索引集。
   * 查看方法：进入「采集项详情」页面，找到已创建的数据源，在基础信息中查看索引集名称。然后在 SDK 代码的 Resource 中设置同样的 `service.name` 值。

4. **检查数据链路与网络**
   * 确认数据链路与页面配置一致。连通性测试方法：`curl -v "https://<API_URL>/v1/logs"`。
